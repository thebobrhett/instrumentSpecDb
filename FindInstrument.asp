...
function editspec(id) {
 if (id=='') {
  alert('You must select an instrument to edit.');
  } else {
  window.location.href='editspec.asp?instrID='+id+'&page_num=1';
  }
}
function printspec(id) {
 if (id=='') {
  alert('You must select an instrument to print.');
  } else {
  window.location.href='printspec.asp?instrID='+id+'&page_num=1';
  }
}
function changespec(id) {
 if (id=='') {
  alert('You must select an instrument for the spec form.');
  } else {
  window.location.href='selectform.asp?instrID='+id+'&page=editspec';
  }
}
function openhelp() {
...
'	Changed "demoed" text to "voided" and changed "showall" filter to use
'	"void_sequence" field instead of "active" field.
...
access = UserAccess("equipment","findinstrument",currentuser)
If access <> "none" Then
	...
	Response.Write "<td style='width:34%' class='center'><input type='button' id='submit1' name='submit1' value='Find' style='font-size:10pt' onclick='doFind();'></td>"
	If Request("showall") <> "" Then
		Response.Write "<td style='width:33%' class='right'><input type='checkbox' id='showall' name='showall' onclick='doFind();' checked />Show Voided Instruments</td>"
	Else
		Response.Write "<td style='width:33%' class='right'><input type='checkbox' id='showall' name='showall' onclick='doFind();' />Show Voided Instruments</td>"
	End If
	Response.Write "</tr>"
	Response.Write "</table>"
	...
	If Request("flowflag") = "true" And criteria <> "" Then
		...
		Response.Write "<tr>"
		Response.Write "<th style='width:20%'>Instrument Name</th>"
		Response.Write "<th style='width:60%'>Instrument Description</th>"
		Response.Write "<th style='width:20%'>Spec Form</th>"
		Response.Write "</tr>"
		Response.Write "<tr>"
		Response.Write "<td colspan='3'><select id='instr_id' size='17' style='font-family:	courier new' onDblClick='printspec(this.value);'>"
		If Not rs.BOF Then
			rs.MoveFirst
			Do While Not rs.EOF
		...
		Response.Write "</tr>"
		Response.Write "<tr>"
	'	Response.Write "<td><button type='button' name='editbutton' id='editbutton' onclick='editspec(document.form1.instr_id.value);'>Edit Spec</button></td>"
		Response.Write "<td>&nbsp;</td>"
		Response.Write "<td><button type='button' name='printbutton' id='printbutton' onclick='printspec(document.form1.instr_id.value);'>View Spec</button></td>"
		Response.Write "<td><button type='button' name='changebutton' id='changebutton' onclick='changespec(document.form1.instr_id.value);'>Change Form</button></td>"
		Response.Write "</tr>"
